...
\tadOtrasOperaciones
    ...
    \tadOperacion{pokemonRareza}{juego/g, pokemon/p}{nat}{$p \in pokemonesTotales(g)$}
...
$pokemonesLibres(g)$ FI}
    ...
    \tadAxioma{pokemonesAAtrapar(g, cpoke, movimientos)}{
    		\IF\ $\emptyset?(cpoke)$ THEN
    			$\emptyset$
    		ELSE 
				{\IF\ (filtrarMovimientos(movimientos, \newline
						obtener(dameUno(cpoke), pokemonesLibres(g))) $ \geq $ 10 $ \wedge $ \newline 
								jugadoresAlRededorPokemon(g, dameUno(cpoke)) $\neq \emptyset$ THEN
    ...
    \tadAxioma{filtrarMovimientos(s, coord)}{
		\IF\ $\emptyset?(s)$ THEN
			$ 0 $
		ELSE
			 {\IF\ (distancia($\pi_2$(prim(s)), coord) $>$ 2) THEN
    ...
    \tadAxioma{borrarPokemones(cpoke, dic\_poke\_coord)}{
		\IF\ $\emptyset$?(cpoke) THEN
			dic\_poke\_coord
		ELSE
			{\IF def?(dameUno(cpoke), dic\_poke\_coord) THEN
				borrarPokemones(\newline
				sinUno(cpoke), borrar(dameUno(cpoke), dic\_poke\_coord)) \newline
			ELSE
				borrarPokemones(sinUno(cpoke), dic\_poke\_coord)
			FI}
		FI
	}
    	% fin auxiliares
    ...
    \tadAxioma{pokemonRareza(g, p)}{100 - (nPokesTipo(p, pokemonesTotales(g))/$\sharp$pokemonesTotales(g))$\ast$100}
